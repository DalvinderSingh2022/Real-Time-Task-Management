...
const DeleteAccountModal = ({ remove, handleLogout }) => {
  ...
  return (
    <>
      {response && <Response />}
      <div
        className="modal flex full_container"
        onClick={!response ? remove : undefined}
      >
        <div
          className={`flex col gap ${authStyles.container} ${modalStyles.container}`}
          onClick={(e) => e.stopPropagation()}
        >
          <div>
            <div className={`w_full text_primary ${authStyles.heading}`}>
              Are you sure?
            </div>
          </div>
